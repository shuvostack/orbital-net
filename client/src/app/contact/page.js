  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // ফর্ম রিসেট করা
        setFormData({ name: '', phone: '', email: '', service: 'নতুন কানেকশন নিতে চাই', message: '' });
      }
    } catch (error) {
      Swal.fire({
        title: 'দুঃখিত!',
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-lg text-zinc-400 max-w-2xl mx-auto font-medium leading-relaxed">
              নতুন কানেকশন, বিল পেমেন্ট বা টেকনিক্যাল সাপোর্ট— আপনার যেকোনো প্রশ্নের উত্তর দিতে আমাদের এক্সপার্ট টিম সবসময় প্রস্তুত।
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {contactCards.map((card, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.15 }} className="bg-zinc-900/50 border border-zinc-800 p-8 rounded-3xl hover:bg-zinc-800/80 transition-all duration-300 hover:-translate-y-1.5 flex flex-col items-center text-center group">
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors" placeholder="যেমন: মেহেদী হাসান" />
                    </div>
                    {/* Phone */}
                    <div className="space-y-2">
                      <label className="text-zinc-400 text-sm font-bold ml-1">ফোন নাম্বার <span className="text-rose-500">*</span></label>
                      <input type="tel" name="phone" value={formData.phone} onChange={handleChange} required className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors" placeholder="017XXXXXXXX" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Email */}
                    <div className="space-y-2">
                      <label className="text-zinc-400 text-sm font-bold ml-1">ইমেইল অ্যাড্রেস <span className="text-rose-500">*</span></label>
                      <input type="email" name="email" value={formData.email} onChange={handleChange} required className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors" placeholder="your@email.com" />
                    </div>

                    {/* Service Type (Dropdown) */}
                    <div className="space-y-2">
                      <label className="text-zinc-400 text-sm font-bold ml-1">কী ধরনের সহায়তা প্রয়োজন? <span className="text-rose-500">*</span></label>
                      <select name="service" value={formData.service} onChange={handleChange} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors appearance-none cursor-pointer">
                        <option value="নতুন কানেকশন নিতে চাই">নতুন কানেকশন নিতে চাই</option>
                        <option value="ইন্টারনেটে সমস্যা (টেকনিক্যাল সাপোর্ট)">ইন্টারনেটে সমস্যা (টেকনিক্যাল সাপোর্ট)</option>
                        <option value="বিলিং বা পেমেন্ট সংক্রান্ত">বিলিং বা পেমেন্ট সংক্রান্ত</option>
                        <option value="প্যাকেজ পরিবর্তন">প্যাকেজ পরিবর্তন</option>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea name="message" value={formData.message} onChange={handleChange} rows="4" className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3.5 text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-colors resize-none" placeholder="আপনার সমস্যা বা চাহিদার কথা বিস্তারিতভাবে লিখুন..."></textarea>
                  </div>

                  {/* Submit Button */}
                  <button type="submit" disabled={isSubmitting} className="group w-full flex items-center justify-center gap-2 bg-gradient-to-r from-orange-500 to-rose-500 text-white px-8 py-4 rounded-xl font-black text-sm uppercase tracking-wider hover:scale-[1.02] active:scale-95 transition-all shadow-[0_10px_20px_rgba(249,115,22,0.2)] disabled:opacity-70 disabled:cursor-not-allowed">
                    {isSubmitting ? 'মেসেজ পাঠানো হচ্ছে...' : (
                      <>মেসেজ সেন্ড করুন <Send className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>
                    )}
                  </button>
                </form>
              </div>
              ...
              <div className="bg-zinc-950 relative overflow-hidden min-h-[400px] lg:min-h-full flex flex-col">
                ...
                <div className="absolute top-6 right-6 z-20 bg-zinc-900/90 backdrop-blur-md border border-zinc-800 rounded-2xl p-4 shadow-xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-white font-bold text-sm">অফিস সময়সূচী</span>
                  </div>
