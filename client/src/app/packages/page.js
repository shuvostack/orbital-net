...
import { Zap, Server, CheckCircle2, ChevronRight, Home, Briefcase, AlertCircle } from 'lucide-react';
...
export default function PackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        ...
        setError('প্যাকেজ লোড করতে সমস্যা হয়েছে। কিছুক্ষণ পর আবার চেষ্টা করুন।');
        setLoading(false);
      }
    };
    fetchPackages();
  ...
  const smePackages = packages.filter(pkg => pkg.type === 'Corporate' || pkg.type === 'Gaming');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-zinc-900/40 border border-zinc-800 rounded-3xl">
              <h3 className="text-white font-bold text-lg mb-2">কোনো প্যাকেজ পাওয়া যায়নি</h3>
              <p className="text-zinc-500 text-sm">আপাতত এই ক্যাটাগরিতে কোনো প্যাকেজ নেই।</p>
            </div>
          ) : (
            /* Packages Grid */
            <AnimatePresence mode="wait">
              <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center">
                {currentPackages.map((pkg) => (
                  <div key={pkg._id} className={`relative bg-zinc-900/40 rounded-[2.5rem] p-8 md:p-10 transition-all duration-300 hover:-translate-y-2 group ${pkg.popular ? 'border-2 border-orange-500 shadow-[0_0_40px_rgba(249,115,22,0.15)] z-10 scale-[1.02]' : 'border border-zinc-800 hover:border-zinc-700'}`}>
                    {pkg.popular && (
                    ...
                    <ul className="space-y-4 mb-10">
                      {pkg.features && pkg.features.map((feature, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <CheckCircle2 className={`w-5 h-5 shrink-0 mt-0.5 ${pkg.popular ? 'text-orange-500' : 'text-zinc-600 group-hover:text-emerald-500 transition-colors'}`} />
