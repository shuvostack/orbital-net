...
export default function AdminPackagesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `https://orbital-backend-9y6q.onrender.com/api/packages/${editingId}`,
          formattedData,
          config,
        );
        Swal.fire({
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await axios.delete(
            `https://orbital-backend-9y6q.onrender.com/api/packages/${id}`,
            config,
          );
          Swal.fire({
            icon: "success",
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">
            আপনার ইন্টারনেট প্যাকেজগুলো ম্যানেজ করুন
          </p>
        </div>
        <button
          onClick={() => openModal()}
      ...
      <div className="bg-zinc-900/40 rounded-3xl border border-zinc-800 p-6">
        {loading ? (
          ...
          <div className="text-center py-12 text-zinc-500">
            কোনো প্যাকেজ পাওয়া যায়নি। দয়া করে নতুন প্যাকেজ অ্যাড করুন।
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredPackages.map((pkg) => (
              <div
                key={pkg._id}
                className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 relative group hover:border-orange-500/50 transition-all"
              >
                {pkg.popular && (
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-500">
                      ৳{pkg.price}
                    </div>
                ...
                <div className="pt-4 border-t border-zinc-800 flex justify-between gap-2">
                  <button
                    ...
                    className="flex-1 py-2.5 bg-blue-500/10 text-blue-400 font-bold rounded-xl hover:bg-blue-500 hover:text-white transition-colors flex items-center justify-center gap-2"
                  >
                    <Edit className="w-4 h-4" /> এডিট
                  </button>
                  <button
                    onClick={() => handleDelete(pkg._id)}
                    className="flex-1 py-2.5 bg-rose-500/10 text-rose-400 font-bold rounded-xl hover:bg-rose-500 hover:text-white transition-colors flex items-center justify-center gap-2"
                  >
        ...
        {isModalOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="bg-zinc-950 border border-zinc-800 rounded-3xl w-full max-w-2xl relative z-10 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
            >
              <div className="flex justify-between items-center p-6 border-b border-zinc-800 shrink-0">
                <h2 className="text-xl font-bold text-white">
                  {editingId ? "প্যাকেজ এডিট করুন" : "নতুন প্যাকেজ যোগ করুন"}
                </h2>
                <button
                  onClick={() => setIsModalOpen(false)}
              ...
              >
                ...
                <div>
                  <label className="block text-zinc-400 text-xs font-bold uppercase mb-2">
                    প্যাকেজের সুবিধা (কমা দিয়ে লিখুন)
                  </label>
                  <textarea
                    name="features"
                    ...
                    rows="3"
                    className="w-full bg-zinc-900 border border-zinc-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-orange-500"
                    placeholder="e.g. Free BDIX, 24/7 Support, Real IP..."
                  ></textarea>
                </div>
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <label
                    htmlFor="popular"
                    className="text-zinc-300 text-sm font-bold cursor-pointer"
                  >
                    এটি কি পপুলার প্যাকেজ? (ওয়েবসাইটে হাইলাইট হবে)
                  </label>
                </div>
                ...
                <div className="pt-6 flex justify-end gap-3 pb-2 border-t border-zinc-800/50 mt-6">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold bg-orange-500 text-white hover:bg-orange-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSubmitting && (
